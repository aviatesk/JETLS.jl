...
function get_type_inlay_hints(
    ...
    JETLS.iterate_toplevel_tree(st0_top) do st0::JS.SyntaxTree
        ctx = @something JETLS.build_inferred_context_at(
        ...
        JETLS.collect_type_inlay_hints!(
            hints, st0, ctx, fi, range, JETLS.LSPostProcessor();
            maxdepth, maxwidth)
    end
    return hints
end
...
@testset HierarchicalTestSet "type inlay hints" begin
    ...
    # Comprehension/`map` lambdas: `K"generator"` / `K"filter"` anchors are suppressed and
    # the iteration binding emits per-var hints like a for-loop.
    # Untyped iter vars also assert an ideal output via `@test_broken` — swap to `@test`
    # when `most_general_argtypes`'s `Tuple{Any}` is refined to the iterator's eltype.
    @testset "comprehension expressions" begin
        @testset "untyped iter var" begin
            # Identity comprehension — JL specializes to `Generator(identity, xs)` without
            # an OC, so the iter-var hint depends on separate eltype-surfacing logic, not
            # `most_general_argtypes`. The `@test_broken` ideal output is here as a
            # reference for that future fix.
            let code = """
                    let xs = rand(3)
                        [x for x in xs]
                    end
                ...
                @test apply_inlay_hints(code, get_type_inlay_hints(code)) == """
                    ...
                    """
                @test_broken apply_inlay_hints(code, get_type_inlay_hints(code)) == """
                    let xs = rand(3)::Vector{Float64}
            ...
            let code = """
                ...
                @test_broken apply_inlay_hints(code, get_type_inlay_hints(code)) == """
                    let xs = rand(3)::Vector{Float64}
                        sum(x::Float64 for x::Float64 in xs::Vector{Float64})::Float64
                    end
                    """
            end
        ...
        @testset "typed iter var" begin
            ...
            # bare `Vector` (separate eltype-flatten gap, marked as broken).
            let code = """
                    let xs = [1, 2, 3], ys = [1.0]
    ...
    @testset "method definitions" begin
        ...
        # These macros are routed through transparent new-style macro definitions,
        # so the inner funcdef's return-type and body annotations land precisely.
        # The macrocall itself doesn't emit a hint (declaration-wrapping macros have no
        # value-type to display) — `is_funcdef_decl` detects the shape and the postorder
        # skips emission for the macrocall anchor.
    ...
    @testset "closures handling" begin
        ...
        # See `test_TypeAnnotation.jl` for how captures resolve precisely.
        ...
        # See `test_TypeAnnotation.jl` for the `::T` rt resolution.
        ...
        # See `test_TypeAnnotation.jl` for the box-erasure limitation.
        ...
        # collapses to `Any` — recorded as `@test_broken` for the precise rt.
        ...
        # `sig_ranges` must cover every comma-separated parameter so none of
        # them pick up `::Any` hints inside.
        ...
        # `map`'s result widens to `Vector` (not `Vector{$Int}`) — recorded
        # as `@test_broken` for the precise element type.
    ...
    # module-path / binding syntax — emitting `::Any` on the statement or
    # any identifier inside is pure noise. Source must round-trip unchanged.
    ...
    @testset "decorative parens" begin
        ...
        # — using the existing parens to keep `.foo` binding outside the type assertion.
