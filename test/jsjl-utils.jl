...
function jldebug(mod::Module, st0_in::JL.SyntaxTree, stop::Int=5)
    ...
    stop = stop - 1; stop < 0 && return; ctx1, st1 = JL.expand_forms_1(mod, st0_in, true, Base.get_world_counter())
...
function jldebug(mod::Module, s::AbstractString, stop::Int=5)
    global st0 = jlparse(s; rule=:statement)
    jldebug(mod, st0, stop)
end
