...
@testset "Multi-position / composition behaviors" begin
    ...
    # branch should pick up its branch-narrowed type (Int / String). When
    # `xxx` appears as a call argument, `annotate_types!` currently uses
    # `CC.argextype(SlotNumber)` which returns `slottypes[id]` — the joined
    # post-inference type — so the narrowing is lost at every such use
    # site. Recorded as `@test_broken` to flip when slot reads pick up
    # per-statement narrowed types (e.g. via `frame.bb_vartables`).
    @testset "narrowed type across control flow" begin
        let code = """
            ...
            @test_broken any(t -> widenconst(t) === Int, types)
            @test_broken any(t -> widenconst(t) === String, types)
        end
    end
end
...
@testset "Pipeline-level edge cases" begin
    ...
    # Top-level bare assignment `x = sin(1.0)` (no enclosing `let`/function)
    # currently doesn't propagate the inferred RHS back to a `:type` on the
    # surface `K"call"`: the toplevel chunk lowers `sin(1.0)` to an SSA
    # statement whose `stmttree` doesn't share the surface call's byte
    # range, so the annotation lands elsewhere. Recorded as `@test_broken`
    # so this flips when the top-level annotation path is tightened.
    @testset "top-level bare assignment RHS" begin
        let code = "x = sin(1.0)"
            _, inferred = type_annotate(code)
            @test_broken widenconst(get_type_for_range(
                inferred, range_of(code, "sin(1.0)"))) === Float64
        end
    end
end
