...
function print_help()::Nothing
    println("""Usage: julia --project=scripts scripts/analyze-heapsnapshot.jl
           [OPTIONS] HEAPSNAPSHOT

    Analyze a V8 heap snapshot generated by JETLS and summarize memory use by
    object type.

    Arguments:
      HEAPSNAPSHOT      Path to a .heapsnapshot file

    Options:
      -h, --help        Show this help message and exit
      --top=N           Show the top N entries (default: 50)
      --retainers=NAME  Show retainers for nodes whose name contains NAME

    Example:
      julia --project=scripts scripts/analyze-heapsnapshot.jl snapshot.heapsnapshot""")
end
