...
# HACK we probably need to make this interface "proper"-ish
function JET.ToplevelAbstractAnalyzer(interp::LSInterpreter, concretized::BitVector, world::UInt)
    @assert isdefined(interp, :state) "Invalid top-level analysis"
    # refresh `interp.analyzer.analyzed_modules`
    empty!(interp.analyzer.analyzed_modules)
    for (_, analyzed_file_info) in interp.state.res.analyzed_files
        for module_range_info in analyzed_file_info.module_range_infos
            push!(interp.analyzer.analyzed_modules, last(module_range_info))
        end
    end
    return @invoke JET.ToplevelAbstractAnalyzer(interp::JET.ConcreteInterpreter, concretized::BitVector, world::UInt)
end
