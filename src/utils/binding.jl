...
function select_inferrable_binding(
    ...
    st0 = @something greatest_local(st0_top, offset) return nothing # nothing we can lower
    (; ctx3, st3) = try
        jl_lower_for_scope_resolution(mod, st0; soft_scope, trim_error_nodes=false, recover_from_macro_errors=false)
    catch err
        JETLS_DEBUG_LOWERING && @warn "Error in lowering ($caller)" err
        JETLS_DEBUG_LOWERING && Base.show_backtrace(stderr, catch_backtrace())
        return nothing
    end
    binding = @something _select_target_binding(ctx3, st3, offset) return nothing
