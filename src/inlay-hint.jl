...
function type_inlay_hints!(
    ...
    iterate_toplevel_tree(st0_top) do st0::SyntaxTreeC
        ...
        ctx = @something build_inferred_context_at(
            ...
            world, caller="type_inlay_hints!") return nothing
        collect_type_inlay_hints!(
            inlay_hints, st0, ctx, fi, range, postprocessor, nontrivia_index)
    end
    return inlay_hints
end
...
function collect_type_inlay_hints!(
    ...
    # - `verbatim_ranges`: spans of user-written verbatim syntax — declaration
    #   signatures (`function f(x::T)::U`, `struct Foo{T} <: AbstractVector{T}`,
    #   …), the RHS of `::`, and `using` / `import` statements. The postorder
    #   pass skips any node whose byte range falls inside one — the user
    #   already wrote what's there.
    ...
    traverse(st0) do node::SyntaxTreeC
        ...
        if JS.kind(node) === JS.K"->" && JS.numchildren(node) >= 1
            # Lambda parameter list (`do y::T` / `(y::T) -> …` / no-paren `x -> …`) is a
            # signature — same treatment as funcdef sig: suppress hints inside.
            # The no-paren single-Identifier form additionally would otherwise pick up the
            # OC binding's `PartialOpaque` `K"slot"` that anchors at the parameter span,
            # surfacing as a misleading `x::OpaqueClosure{…}` hint.
            params = node[1]
        ...
        if JS.kind(node) === JS.K"=" && JS.numchildren(node) >= 1
            ...
            if JS.kind(lhs) === JS.K"Identifier"
                ...
                # `where`-wrapped or `::T`-annotated). `funcdef_verbatim_range`
        ...
        # Comprehension iteration: `[expr for var = iter]` parses as
        # `K"comprehension"(K"generator"(expr, K"="(var, iter)))`, with an optional
        # `K"filter"` wrapping the binding when an `if cond` is present.
        # Mirror the for-loop handling so the iter var(s) get a clean per-var hint instead
        # of a wrapper-tuple `::Any`. We emit per-var hints via `emit_for_loop_var_hints!`
        # in the K"generator"/K"filter" postorder branch below.
        ...
        # `using`/`import` statements — the entire span is user-written
        # module-path / binding syntax with no inferred result to surface.
        # Register as a sig range so the statement itself and every nested
        # identifier (`using A.B: c, d as e`) skip the postorder visit.
    ...
    #   5. Nodes whose range lies inside a `verbatim_ranges` entry — skip.
    ...
    traverse(st0, #=postorder=#true) do node::SyntaxTreeC
        ...
        # Macros that decorate a function definition (`@inline`, `@noinline`,
        # `Base.@propagate_inbounds`, …) wrap the funcdef rather than producing
        # a value-typed expression; emitting `(@inline f(...) = body)::T`
        # would mis-frame the macro as if it has a meaningful runtime value.
        ...
        # Comprehension iteration — emit per-var hints on the binding LHS, then
        # bail out so the K"generator"/K"filter" anchor itself doesn't pick up
        # the internal `Generator{…}` / `Filter{…}` lowering type.
...
# Funcdef-shape detector for `K"macrocall"` suppression: matches long-form
# `function f(...) … end` / `macro m(...) … end` and short-form `f(args) = body`
# (possibly `where`-wrapped or `::T`-annotated). Looser than `funcdef_call_node`
# — accepts user-typed return types — because we just want to know whether the
# wrapped expression is a definition shape, not whether to emit a return-type
# hint on it.
...
# Locate the `K"="` iteration binding inside a `K"generator"` or `K"filter"`.
# `K"generator"`'s second child is either the binding itself or a `K"filter"` wrapping it
# (when an `if cond` is present); `K"filter"`'s second child is the binding. Returns the
# `K"="` node, or `nothing` if the shape doesn't match.
...
# Used to populate `verbatim_ranges` so where-clause bounds (e.g. `T <: Number`) don't pick up
...
# Recursively register byte ranges inside an iteration LHS so the regular
# postorder visit skips them — `emit_for_loop_var_hints!` emits a single
# hint per declared variable instead. Registers each `K"Identifier"`
# *and* the wrapping `K"tuple"` / `K"parameters"` nodes: in for-loops the
# wrapper byte range typically has no typed lowered node and would be
# silently skipped, but in comprehensions the OC body's slot read sits at
# the wrapper range and would otherwise emit a redundant `(x, y)::Any`.
...
# `verbatim_ranges` holds byte ranges whose contents are user-written verbatim syntax that
# the inlay-hint pass should leave untouched — declaration signatures (`function f(...)` /
# `macro m(...)`, lambda parameter tuples, struct / abstract / primitive name expressions),
# the RHS of `::`, and `using` / `import` statements. Anything inside one of these ranges
# (type annotations, default-value calls, `where` constraints, parametric type names,
# module-path / binding syntax) is something the user already wrote out, so emitting
# `::Any` hints there is pure noise.
in_verbatim_range(byterng::UnitRange{<:Integer}, verbatim_ranges::Set{UnitRange{Int}}) =
