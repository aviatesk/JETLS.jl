    ...
    if frame.linfo === interp.topmi
        for i = 1:length(frame.src.code)
            ...
            elseif stmt isa ReturnNode
                ...
                JL.setattr!(interp.toptree[1][i]; type=rettyp)
            end
        end
    end
    return ret
...
function _infer_toplevel_tree(st0::JL.SyntaxTree, context_module::Module; limit_aggressive_inference::Bool=false)
    JS.kind(st0) in JS.KSet"toplevel module" &&
        error("infer_toplevel_tree expects expressions that are not toplevel or module kinds")
