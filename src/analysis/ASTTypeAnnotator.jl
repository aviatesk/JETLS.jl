...
CC.transform_result_for_cache(::ASTTypeAnnotator, ::CC.InferenceResult, edges::Core.SimpleVector) = nothing
...
function CC.abstract_eval_globalref(interp::ASTTypeAnnotator, g::GlobalRef, bailed::Bool,
...
function CC.concrete_eval_eligible(interp::ASTTypeAnnotator, @nospecialize(f),
...
function infer_method(interp::ASTTypeAnnotator, e::Expr, sstate::CC.StatementState, sv::CC.InferenceState)
    ret = _infer_method(interp, e, sstate, sv)
    if ret === nothing
        return nothing
    end
    @info "Inner method inference failed" reason=ret
    return nothing
end
...
@inline function CC.abstract_eval_basic_statement(
        ...
        result::Union{Nothing,CC.Future{CC.RTEffects}}
    )
    if e.head === :method && interp.topmi === sv.linfo
        infer_method(interp, e, sstate, sv)
    end
    return @invoke CC.abstract_eval_basic_statement(
...
@inline function CC.abstract_eval_basic_statement(
    interp::ASTTypeAnnotator, @nospecialize(stmt), sstate::CC.StatementState, frame::CC.InferenceState,
    result::Union{Nothing,CC.Future{CC.RTEffects}})
    result = @invoke CC.abstract_eval_basic_statement(
        interp::CC.AbstractInterpreter, stmt::Any, sstate::CC.StatementState, frame::CC.InferenceState,
        result::Union{Nothing,CC.Future{CC.RTEffects}})
    if result isa CC.AbstractEvalBasicStatementResult
        result = CC.AbstractEvalBasicStatementResult(
            result.rt, result.exct, result.effects, result.changes, result.refinements,
...
function CC.finishinfer!(frame::CC.InferenceState, interp::ASTTypeAnnotator, cycleid::Int)
    ...
    if frame.linfo === interp.topmi
        for i = 1:length(frame.src.code)
            ...
            JL.setattr!(interp.toptree[1][i]; type=stmttype)
            if Meta.isexpr(stmt, :(=))
                lhs = stmt.args[1]
                if lhs isa Core.SlotNumber
                    JL.setattr!(interp.toptree[1][i][1]; type=stmttype)
                end
            end
        end
    end
...
function _infer_toplevel_tree(st0::JL.SyntaxTree, context_module::Module; limit_aggressive_inference::Bool=false)
    if JS.kind(st0) in JS.KSet"toplevel module"
        return nothing
    end

    lwrst = try
        JL.lower(context_module, st0)
    catch e
        @error "Lowering failed" e
        return nothing
    end |> prepare_type_attr
    lwr = JL.to_lowered_expr(context_module, lwrst)

    Meta.isexpr(lwr, :thunk) || error("Unexpected lowering")
